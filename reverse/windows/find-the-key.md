...
|------|----------|------|----------|
| **IDA Pro** | Windows/Linux/macOS | Commercial | Industry-standard disassembler with decompiler |
| **Ghidra** | Windows/Linux/macOS | Free (NSA) | Open-source RE suite with decompiler |
| **x64dbg** | Windows | Free | User-mode x64 debugger with GUI |
| **WinDbg** | Windows | Free | Microsoft kernel/user debugger |
| **radare2 / rizin** | Windows/Linux/macOS | Free | Command-line RE framework |
| **Binary Ninja** | Windows/Linux/macOS | Commercial | Modern RE platform with HLIL |
| **HxD** | Windows | Free | Hex editor for manual byte inspection |
| **010 Editor** | Windows/Linux/macOS | Commercial | Hex editor with template-based parsing |
| **CyberChef** | Web | Free | Online tool for encoding/decoding/cryptanalysis |
| **FLOSS** | Windows/Linux/macOS | Free | FireEye's advanced string extraction (handles obfuscation) |
...
1. **String artifacts are the breadcrumbs of reverse engineering.** The regex pattern `^[A-Z]{2}-[A-Z]{2}$` immediately revealed the Magic Number format, and the encoded payload `TM3N[YkM1htam3>` was the primary cryptographic target. Always start with strings — they often reveal 70% of what you need to know.

2. **DWORD-stored characters are a common but easily defeated obfuscation.** Storing individual ASCII characters as 4-byte integers (with 3 zero-padding bytes) hides them from simple string extraction tools. However, this technique is trivially defeated by examining the binary in a hex editor or by tracing the code that reads these addresses. The key lesson is to always use hex dumps in addition to string tools.

3. **Single-byte XOR is never secure.** With only 256 possible keys, brute force is instantaneous. Even without understanding the key derivation logic, trying all 256 keys and checking which one produces valid Base64 (and then printable ASCII after decoding) uniquely identifies the correct key. Multi-byte XOR keys or more complex ciphers (AES, ChaCha20) would be needed for any real security.

4. **The Magic Number dual-roles as input and cryptographic parameter.** This is a clever design choice that ties the "password" to the decryption process. Without knowing the correct Magic Number (or at least one that produces the correct XOR key), the encoded payload cannot be decrypted. This adds a layer of complexity beyond a simple password check.

5. **Base64 after XOR is a common two-layer encoding in CTFs.** XOR encryption alone produces binary data that may not be printable. Base64 encoding the XOR result converts it to printable ASCII, making it easier to store as a string constant in the binary. However, this also provides a useful validation criterion during brute force — only the correct XOR key will produce valid Base64 characters.

6. **The prefix/split technique adds indirection.** By storing the first 5 characters of the encoded string as separate DWORDs and the remaining 15 characters as a contiguous string, the author made the full encoded string harder to discover. This is a simple but effective obfuscation that would fool automated string-based analysis tools.

### 15.2 Methodological Lessons

1. **Start with reconnaissance, not with disassembly.** Running `file`, `strings`, and `hexdump` before opening IDA/Ghidra provides context that makes the subsequent RE work much faster. You should know what you are looking for before diving into assembly code.

2. **Form hypotheses early and test them.** After the initial reconnaissance, the hypothesis "prefix + payload → XOR decrypt → Base64 decode" was formed and tested in Python before the full disassembly was completed. This iterative approach is more efficient than trying to understand every instruction before forming a theory.

3. **Multiple approaches lead to the same answer.** Whether through full static RE, XOR brute force, or dynamic debugging with breakpoints, the key `{Act1va7i0n}` can be recovered. The redundancy of approaches provides confidence in the result and serves as a cross-validation mechanism.

4. **CTF challenges often use leet speak and wordplay.** The key `{Act1va7i0n}` is leet speak for "Activation," which is a common theme in crackme-style challenges. Being familiar with leet speak conventions helps in recognizing decoded strings that initially appear to be garbled.
...
|-------|----------|-------------|
| PE File Format | [Microsoft PE Specification](https://learn.microsoft.com/en-us/windows/win32/debug/pe-format) | Official documentation of the Portable Executable format |
| x86-64 Assembly | [Intel x64 Instruction Set Reference](https://www.intel.com/content/www/us/en/architecture-and-technology/64-ia-32-architectures-software-developer-manual.html) | Complete reference for all x86-64 instructions |
| XOR Cryptanalysis | [XOR Cipher - Wikipedia](https://en.wikipedia.org/wiki/XOR_cipher) | Mathematical properties and known-plaintext attacks |
| Base64 Encoding | [RFC 4648 - The Base16, Base32, and Base64 Data Encodings](https://datatracker.ietf.org/doc/html/rfc4648) | Official specification of the Base64 algorithm |
| Reverse Engineering | [Practical Reverse Engineering](https://www.nostarch.com/reverse) by Bruce Dang et al. | Comprehensive RE textbook covering x86, ARM, and obfuscation |
| Ghidra | [NSA Ghidra Documentation](https://ghidra.re/) | Free, open-source reverse engineering suite |
| Crackme Challenges | [Crackmes.one](https://crackmes.one/) | Community database of reverse engineering challenges |
| Leet Speak | [Leet - Wikipedia](https://en.wikipedia.org/wiki/Leet) | History and conventions of leet speak notation |
