const ASCII_BANNER = "  ___        _____     _          ___  ___      _____ \n / _ \\      |  _  |   | |         |  \\/  |     |  _  |\n/ /_\\ \\_ __ | |/' | __| | _____  _| .  . | ___ | |/' |\n|  _  | '_ \\|  /| |/ _` |/ _ \\ \\/ / |\\/| |/ _ \\|  /| |\n| | | | |_) | |_/ / (_| |  __/>  <| |  | |  __/\\ |_/ /\n\\_| |_/ .__/ \\___/ \\__,_|\\___/_/\\_\\_|  |_/ \\___| \\___/\n      | |                                             \n      |_|";
const NEOFETCH_ART = "     ██████╗ ██╗     ███╗   ███╗\n    ██╔════╝ ██║     ████╗ ████║\n    ██║  ███╗██║     ██╔████╔██║\n    ██║   ██║██║     ██║╚██╔╝██║\n    ╚██████╔╝███████╗██║ ╚═╝ ██║\n     ╚═════╝ ╚══════╝╚═╝     ╚═╝";

/* === HELPERS: SECURE ESCAPING === */
...
/* === PARTICLE === */
(function(){var c=document.getElementById('particle-bg'),ctx=c.getContext('2d'),pts=[];function resize(){c.width=innerWidth;c.height=innerHeight;pts=[];for(var i=0;i<60;i++)pts.push({x:Math.random()*c.width,y:Math.random()*c.height,vx:(Math.random()-0.5)*0.4,vy:(Math.random()-0.5)*0.4,r:Math.random()*1.5+0.5});}resize();addEventListener('resize',resize);function draw(){ctx.clearRect(0,0,c.width,c.height);for(var i=0;i<pts.length;i++){var p=pts[i];p.x+=p.vx;p.y+=p.vy;if(p.x<0||p.x>c.width)p.vx*=-1;if(p.y<0||p.y>c.height)p.vy*=-1;ctx.beginPath();ctx.arc(p.x,p.y,p.r,0,Math.PI*2);ctx.fillStyle='rgba(168,85,247,0.25)';ctx.fill();for(var j=i+1;j<pts.length;j++){var q=pts[j],dx=p.x-q.x,dy=p.y-q.y,d=Math.sqrt(dx*dx+dy*dy);if(d<140){ctx.beginPath();ctx.moveTo(p.x,p.y);ctx.lineTo(q.x,q.y);ctx.strokeStyle='rgba(168,85,247,'+(0.08*(1-d/140))+')';ctx.stroke()}}}}setInterval(draw,40);})();

function updateClocks(){var n=new Date(),t=n.toLocaleTimeString('en-US',{hour12:false}),d=n.toLocaleDateString('en-US',{weekday:'short',month:'short',day:'numeric'});var e1=document.getElementById('header-clock'),e2=document.getElementById('status-time');if(e1)e1.textContent=d+' '+t;if(e2)e2.textContent=t;}
...
/* === FILESYSTEM === */
async function loadFileSystem(){var fs={};try{var r=await fetch('manifest.json');if(!r.ok)throw new Error('HTTP '+r.status);var m=await r.json();if(m.directories)m.directories.forEach(function(d){if(!fs[d])fs[d]={};});var st=m.structure||{};for(var dir in st){if(!fs[dir])fs[dir]={};var files=st[dir];if(Array.isArray(files)){for(var i=0;i<files.length;i++){var fn=files[i];if(!fn)continue;try{var r2=await fetch('reverse/'+dir+'/'+fn);fs[dir][fn]=r2.ok?await r2.text():'[ERROR] Not found';}catch(e){fs[dir][fn]='[ERROR] Load failed';}}}}}catch(e){console.error('Manifest error:',e);}return {reverse:fs};}

var shellState={path:[],history:[],historyIndex:-1,cmdCount:0,completion:{input:'',matches:[],index:0}};
var fileSystem={};
var currentFileContent="";

function getCurrentDirObj(){var c=fileSystem["reverse"];if(!c)return null;for(var i=0;i<shellState.path.length;i++){var s=shellState.path[i];if(c[s]&&typeof c[s]==='object')c=c[s];else return null;}return c;}
...
function showNotification(message,type,Duration){var t=type||'info',d=Duration||2500,con=document.getElementById('toast-container'),el=document.createElement('div');el.className='toast toast-'+t;var icons={success:'&#10003;',error:'&#10005;',info:'&#8505;',warning:'&#9888;'};el.innerHTML=('<span>'+icons[t]+'</span>')+escapeHtml(message);con.appendChild(el);setTimeout(function(){el.classList.add('hiding');setTimeout(function(){el.remove();},200);},d);}
...
function updateStatus(){document.getElementById('status-cmds').textContent=shellState.cmdCount+' cmds';document.getElementById('sb-cmds').textContent=shellState.cmdCount;}

/* === COMMANDS (FIXED) === */
var commands={
  help:function(){print('<div style="line-height:1.8;"><span class="gradient-text font-bold text-sm">  AVAILABLE COMMANDS</span><br><br><span class="text-slate-500 font-semibold">Navigation</span><br>  <span class="text-cyan-300">ls [dir]</span>      <span class="text-slate-600">List directory</span><br>  <span class="text-cyan-300">cd [dir]</span>      <span class="text-slate-600">Change directory</span><br>  <span class="text-cyan-300">pwd</span>           <span class="text-slate-600">Working directory</span><br>  <span class="text-cyan-300">tree [dir]</span>    <span class="text-slate-600">Directory tree</span><br>  <span class="text-cyan-300">find [pat]</span>    <span class="text-slate-600">Find files</span><br><br><span class="text-slate-500 font-semibold">Files</span><br>  <span class="text-cyan-300">cat [file]</span>    <span class="text-slate-600">Raw content</span><br>  <span class="text-cyan-300">view [file]</span>   <span class="text-slate-600">Markdown viewer</span><br>  <span class="text-cyan-300">head [file]</span>   <span class="text-slate-600">First N lines</span><br>  <span class="text-cyan-300">tail [file]</span>   <span class="text-slate-600">Last N lines</span><br>  <span class="text-cyan-300">wc [file]</span>     <span class="text-slate-600">Word/line count</span><br>  <span class="text-cyan-300">grep [p] [f]</span>  <span class="text-slate-600">Search in file</span><br>  <span class="text-cyan-300">file [name]</span>   <span class="text-slate-600">Detect type</span><br><br><span class="text-slate-500 font-semibold">System</span><br>  <span class="text-cyan-300">whoami</span>        <span class="text-slate-600">Current user</span><br>  <span class="text-cyan-300">uname [-a]</span>    <span class="text-slate-600">System info</span><br>  <span class="text-cyan-300">neofetch</span>      <span class="text-slate-600">System + art</span><br>  <span class="text-cyan-300">date</span>          <span class="text-slate-600">Date/time</span><br>  <span class="text-cyan-300">uptime</span>        <span class="text-slate-600">Uptime</span><br>  <span class="text-cyan-300">ps</span>            <span class="text-slate-600">Processes</span><br>  <span class="text-cyan-300">ifconfig</span>      <span class="text-slate-600">Network</span><br>  <span class="text-cyan-300">ping [host]</span>   <span class="text-slate-600">Ping</span><br><br><span class="text-slate-500 font-semibold">Utilities</span><br>  <span class="text-cyan-300">echo [text]</span>   <span class="text-slate-600">Print text</span><br>  <span class="text-cyan-300">history [-c]</span>  <span class="text-slate-600">History</span><br>  <span class="text-cyan-300">clear</span>         <span class="text-slate-600">Clear</span><br>  <span class="text-cyan-300">banner</span>        <span class="text-slate-600">Welcome banner</span><br>  <span class="text-cyan-300">calc [expr]</span>   <span class="text-slate-600">Calculator</span><br>  <span class="text-cyan-300">base64</span>        <span class="text-slate-600">Encode/decode</span><br></div>','raw',true);},
  clear:function(){var o=document.getElementById('output');o.style.opacity='0';setTimeout(function(){o.innerHTML='';o.style.opacity='1';showNotification('Terminal cleared','info');},150);},
  whoami:function(){print('<span class="p-user">'+CONFIG.USER+'</span>','success');},
  pwd:function(){print(getFullPath(),'info');showNotification('Current: '+getFullPath(),'info');},
  echo:function(){var t=Array.from(arguments).join(' ');t=t.replace(/\$USER/g,CONFIG.USER).replace(/\$HOST/g,CONFIG.HOST).replace(/\$PWD/g,getFullPath());print(escapeHtml(t));},
  date:function(){print(new Date().toString(),'info');},
  uptime:function(){var s=Math.floor(Date.now()/1000-CONFIG.UPTIME),h=Math.floor(s/3600),m=Math.floor((s%3600)/60),ss=s%60;print('up '+h+'h '+m+'m '+ss+'s, load: 0.'+(Math.floor(Math.random()*30)+10),'info');},
  ...
  ls:function(dir){var t=dir?resolvePath(dir):getCurrentDirObj();if(!t){showNotification("Directory not found: "+escapeHtml(dir),'error');print("ls: '"+escapeHtml(dir)+"': not found",'error');return;}var items=Object.keys(t);if(!items.length){print('<span class="text-slate-600">(empty)</span>','raw');return;}var out='<div class="flex flex-wrap gap-x-3 gap-y-1">';var base=getCurrentRelativePath();items.forEach(function(n){var isDir=typeof t[n]==='object';var disp=escapeHtml(n)+(isDir?'/':'');var itemPath=base?(base+'/'+n):n;var safePath=escapeAttr(itemPath);out+='<span class="'+(isDir?'out-dir':'out-file')+'" style="cursor:pointer" onclick="sidebarClick(\''+safePath+'\','+isDir+')">'+(isDir?'&#128193;':'&#128196;')+' '+disp+'</span>';});out+='</div>';print(out);showNotification('Listed '+items.length+' items','success');},
  cd:function(dir){if(!dir||dir==='~'||dir==='/'){shellState.path=[];}else if(dir==='..'){if(shellState.path.length>0)shellState.path.pop();}else if(dir.startsWith('/')){var parts=dir.split('/').filter(function(x){return x;}),cur=fileSystem,np=[];for(var i=0;i<parts.length;i++){if(cur[parts[i]]&&typeof cur[parts[i]]==='object'){cur=cur[parts[i]];np.push(parts[i]);}else{showNotification("Directory not found: "+escapeHtml(dir),'error');print("cd: '"+escapeHtml(dir)+"': not found",'error');return;}}shellState.path=np;}else{var cdObj=getCurrentDirObj(),clean=dir.replace(/\/$/,'');if(cdObj&&cdObj[clean]&&typeof cdObj[clean]==='object')shellState.path.push(clean);else{showNotification("Directory not found: "+escapeHtml(dir),'error');print("cd: '"+escapeHtml(dir)+"': not found",'error');return;}}updatePrompt();updateSidebar();showNotification('Changed to '+getFullPath(),'info');},
  cat:function(file){if(!file){showNotification('Usage: cat [file]','info');print('usage: cat [file]','info');return;}var t=resolvePathToTarget(file);if(t===null){showNotification("File not found: "+escapeHtml(file),'error');print("cat: '"+escapeHtml(file)+"': not found",'error');return;}if(typeof t==='object'){showNotification(escapeHtml(file)+" is a directory",'error');print("cat: '"+escapeHtml(file)+"': is a directory",'error');return;}print('<pre class="whitespace-pre-wrap text-xs leading-relaxed out-file">'+escapeHtml(t)+'</pre>','raw');showNotification('Read '+escapeHtml(file),'success');},
  head:function(file,n){if(!file){showNotification('Usage: head [file] [n]','info');print('usage: head [file] [n]','info');return;}var lines=parseLineCount(n,20),t=resolvePathToTarget(file);if(t===null){showNotification("File not found: "+escapeHtml(file),'error');print("head: '"+escapeHtml(file)+"': not found",'error');return;}if(typeof t==='object'){showNotification('Cannot read directory','error');print("head: directory",'error');return;}var s=t.split('\n').slice(0,lines).join('\n');print('<pre class="whitespace-pre-wrap text-xs leading-relaxed out-file">'+escapeHtml(s)+'</pre>','raw');showNotification('Read first '+lines+' lines of '+escapeHtml(file),'success');},
  tail:function(file,n){if(!file){showNotification('Usage: tail [file] [n]','info');print('usage: tail [file] [n]','info');return;}var lines=parseLineCount(n,20),t=resolvePathToTarget(file);if(t===null){showNotification("File not found: "+escapeHtml(file),'error');print("tail: '"+escapeHtml(file)+"': not found",'error');return;}if(typeof t==='object'){showNotification('Cannot read directory','error');print("tail: directory",'error');return;}var all=t.split('\n'),s=lines===0?'':all.slice(-lines).join('\n');print('<pre class="whitespace-pre-wrap text-xs leading-relaxed out-file">'+escapeHtml(s)+'</pre>','raw');showNotification('Read last '+lines+' lines of '+escapeHtml(file),'success');},
  wc:function(file){if(!file){showNotification('Usage: wc [file]','info');print('usage: wc [file]','info');return;}var t=resolvePathToTarget(file);if(t===null){showNotification("File not found: "+escapeHtml(file),'error');print("wc: '"+escapeHtml(file)+"': not found",'error');return;}if(typeof t==='object'){showNotification('Cannot count directory','error');print("wc: directory",'error');return;}var l=countLines(t),w=countWords(t);print('  <span class="text-cyan-300">'+l+'</span>  <span class="text-cyan-300">'+w+'</span> <span class="text-cyan-300">'+t.length+'</span> <span class="out-file">'+escapeHtml(file)+'</span>','raw');showNotification(escapeHtml(file)+': '+l+' lines, '+w+' words','success');},
  grep:function(pattern,file){if(!pattern||!file){showNotification('Usage: grep [pattern] [file]','info');print('usage: grep [pattern] [file]','info');return;}var t=resolvePathToTarget(file);if(t===null){showNotification("File not found: "+escapeHtml(file),'error');print("grep: '"+escapeHtml(file)+"': not found",'error');return;}if(typeof t==='object'){showNotification('Cannot search directory','error');print("grep: directory",'error');return;}var lines=t.split('\n').filter(function(l){return l.toLowerCase().includes(pattern.toLowerCase());});if(!lines.length){showNotification('No matches for "'+escapeHtml(pattern)+'"','warning');print('no matches','dim');return;}var s=lines.map(function(l){return escapeHtml(l);}).join('\n');var re=new RegExp('('+pattern.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')+')','gi');s=s.replace(re,'<span class="bg-purple-500/30 text-purple-300 font-bold">$1</span>');print('<pre class="whitespace-pre-wrap text-xs leading-relaxed">'+s+'</pre>','raw');showNotification('Found '+lines.length+' matches','success');},
  file:function(name){if(!name){showNotification('Usage: file [name]','info');print('usage: file [name]','info');return;}var t=resolvePathToTarget(name);if(t===null){showNotification("File not found: "+escapeHtml(name),'error');print("file: '"+escapeHtml(name)+"': not found",'error');return;}var ext=name.includes('.')?name.split('.').pop().toLowerCase():'';var types={md:'Markdown',txt:'ASCII text',json:'JSON data',js:'JavaScript',py:'Python',sh:'Shell script',html:'HTML',css:'CSS',log:'Log file'};print(escapeHtml(name)+': '+(types[ext]||'data'),'info');showNotification(escapeHtml(name)+' detected as '+(types[ext]||'data'),'success');},
  view:function(file){
    if(!file){showNotification('Usage: view [file]','info');print('usage: view [file]','info');return;}
    var t=resolvePathToTarget(file);
    if(t===null){showNotification("File not found: "+escapeHtml(file),'error');print("view: '"+escapeHtml(file)+"': not found",'error');return;}
    if(typeof t==='object'){showNotification(escapeHtml(file)+" is a directory",'error');print("view: '"+escapeHtml(file)+"': is directory",'error');return;}
    ...
    showNotification('Opened '+escapeHtml(dn),'success');
  },
  tree:function(dir){var target=dir?resolvePath(dir):getCurrentDirObj();if(!target){showNotification("Directory not found: "+escapeHtml(dir),'error');print("tree: '"+escapeHtml(dir)+"': not found",'error');return;}var lines=[];var build=function(obj,prefix){var keys=Object.keys(obj).sort();keys.forEach(function(k,i){var last=i===keys.length-1,isDir=typeof obj[k]==='object';var conn=last?'\u2514\u2500\u2500 ':'\u251C\u2500\u2500 ';var cls=isDir?'out-dir':'out-file';lines.push('<span class="'+cls+'">'+prefix+conn+escapeHtml(k)+(isDir?'/':'')+'</span>');if(isDir)build(obj[k],prefix+(last?'    ':'\u2502   '));});};build(target,'');print(lines.length?'<div class="leading-relaxed">'+lines.join('<br>')+'</div>':'<span class="text-slate-600">(empty)</span>','raw');showNotification('Tree displayed: '+lines.length+' items','info');},
  find:function(pattern){if(!pattern){showNotification('Usage: find [pattern]','info');print('usage: find [pattern]','info');return;}var results=[];var search=function(obj,path){for(var k in obj){var cp=path?path+'/'+k:k;if(k.toLowerCase().includes(pattern.toLowerCase()))results.push({name:cp,isDir:typeof obj[k]==='object'});if(typeof obj[k]==='object')search(obj[k],cp);}};search(fileSystem.reverse,'');if(!results.length){showNotification("No matches for '"+escapeHtml(pattern)+"'",'warning');print("find: no matches for '"+escapeHtml(pattern)+"'",'error');}else{results.forEach(function(r){print('<span class="'+(r.isDir?'out-dir':'out-file')+'">'+escapeHtml(r.name)+(r.isDir?'/':'')+'</span>','normal',true);});showNotification('Found '+results.length+' matches','success');}},
  history:function(){var a=Array.from(arguments);if(a.includes('-c')){shellState.history=[];shellState.historyIndex=-1;showNotification('History cleared','success');print('History cleared','success');return;}if(!shellState.history.length){print('<span class="text-slate-600">(empty)</span>','raw');return;}shellState.history.forEach(function(c,i){print('  <span class="text-slate-600">'+String(i+1).padStart(4,' ')+'</span>  '+escapeHtml(c),'raw');});showNotification('History: '+shellState.history.length+' entries','info');}
};
...
function toggleRawView(){var e=getModalEls(),isRaw=!e.raw.classList.contains('hidden');if(isRaw){e.raw.classList.add('hidden');e.content.classList.remove('hidden');e.status.textContent='RENDERED';e.rawBtnText.textContent='Raw';showNotification('Switched to rendered view','info');}else{e.content.classList.add('hidden');e.raw.classList.remove('hidden');e.status.textContent='RAW';e.rawBtnText.textContent='Rendered';showNotification('Switched to raw view','info');}}
function copyModalContent(){if(!currentFileContent)return;try{if(!navigator.clipboard||!navigator.clipboard.writeText){showNotification('Copy unavailable in this browser context','error');print('Copy unavailable in this browser context','error');return;}navigator.clipboard.writeText(currentFileContent).then(function(){showNotification('Copied to clipboard','success');print('Copied to clipboard','success');}).catch(function(){showNotification('Copy failed','error');print('Copy failed','error');});}catch(e){showNotification('Copy failed','error');print('Copy failed','error');}}
function downloadCurrentFile(){if(!currentFileContent)return;var title=(document.getElementById('modal-title').textContent||'file').trim(),ext=(document.getElementById('modal-ext-badge').textContent||'.txt').replace(/^\./,'').trim().toLowerCase(),blob=new Blob([currentFileContent],{type:'text/plain'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;var hasExt=ext&&title.toLowerCase().endsWith('.'+ext);var fileName=hasExt||!ext?title:(title+'.'+ext);a.download=fileName;a.click();URL.revokeObjectURL(url);showNotification('Downloaded '+escapeHtml(fileName),'success');print('Downloaded <span class="text-cyan-300">'+escapeHtml(fileName)+'</span>','success');}
function toggleSidebar(){var panel=document.getElementById('sidebar-panel'),backdrop=document.getElementById('sidebar-backdrop');if(!panel||!backdrop||window.innerWidth>768)return;var isOpen=panel.classList.contains('open');panel.classList.toggle('open',!isOpen);backdrop.classList.toggle('active',!isOpen);showNotification(isOpen?'Sidebar closed':'Sidebar opened','info');}
...
function initTerminal(){commands.banner();document.getElementById('cmd').focus();updatePrompt();updateStatus();updateSidebar();}
function revealTerminal(){var app=document.getElementById('terminal-app');if(!app)return;app.classList.remove('opacity-0','pointer-events-none');}
window.addEventListener('resize',function(){if(window.innerWidth>768)closeSidebar();});
window.addEventListener('load',async function(){await playBootSequence();fileSystem=await loadFileSystem();initTerminal();revealTerminal();});
document.addEventListener('click',function(){var m=document.getElementById('file-modal');if(m.classList.contains('hidden'))document.getElementById('cmd').focus();});